...
$query="UPDATE `pv_evaluation` SET "; 
$query2="UPDATE `appel_doffre` SET `etape` = 'CEOT' WHERE `code` = '"; 
$query4="UPDATE `appel_doffre` SET `etape` = 'COP technique' WHERE `code` = '"; 
...
{
     if ($conn->query($query."`num_pv` = '".$_POST["num_pv"]."' WHERE `id_pv` =".$_POST["id_ceot"]) === FALSE) 
        {
            $reply["success"]=FALSE;
...
{
     if ($conn->query($query."`date_pv` = '".$_POST["date_pv"]."' WHERE `id_pv` =".$_POST["id_ceot"]) === FALSE) 
        {
            $reply["success"]=FALSE;
            $reply["message"]=$conn->error;
        }
...
{
     if ($conn->query($query."`file_pv` = '".$_POST["file_pv"]."' WHERE `id_pv` =".$_POST["id_ceot"]) === FALSE) 
        {
            $reply["success"]=FALSE;
            $reply["message"]=$conn->error;
        }
...
{ if($_POST["code"]!=$_POST["ancode"])
     {if ($conn->query($query."`code` = '".$_POST["code"]."' WHERE `id_pv` =".$_POST["id_ceot"]) === FALSE) 
        {
            $reply["success"]=FALSE;
            $reply["message"]=$conn->error;
        }
