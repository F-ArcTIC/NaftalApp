...
$query4="UPDATE `appel_doffre` SET `etape` = 'initiale' WHERE `code` = '"; 
$query5="UPDATE `préqualif` SET `etape` = 'initiale' WHERE `code` = '"; 
$query33="UPDATE `appel_doffre` SET `etat` = 'Ajourné' WHERE `code` = '"; 
$query44="UPDATE `préqualif` SET `etat` = 'Ajourné' WHERE `code` = '"; 
$query22="UPDATE `appel_doffre` SET `etat` = 'En cours' WHERE `code` = '"; 
$query55="UPDATE `préqualif` SET `etat` = 'En cours' WHERE `code` = '"; 
$reply=array();
$reply["success"]=TRUE;
...
if(isset($_POST["Recommandation"]))
{
    ...
    if($_POST["Recommandation"]==1){
                            ...
                            if ($conn->query($query2.$_POST["code"]."'") === FALSE) 
                                ...
                                if ($conn->query($query33.$_POST["code"]."'") === FALSE) 
                                        {
                                            $reply["success"]=FALSE;
                                            $reply["message"]=$con->error;
                                        }
                                        if ($conn->query($query44.$_POST["code"]."'") === FALSE) 
                                        {
                                            $reply["success"]=FALSE;
                                            $reply["message"]=$con->error;
                                        }
	 ...
	 if ($conn->query($query."`code` = '".$_POST["code"]."' WHERE `id_cvt` =".$_POST["id_cvt"]) === FALSE) 
    	{
            ...
            $reply["message"]=$conn->error;
                                        ...
                                        if ($conn->query($query22.$_POST["ancode"]."'") === FALSE) 
                                        {
                                            $reply["success"]=FALSE;
                                            $reply["message"]=$conn->error;
                                        }
                                            ...
                                            if ($conn->query($query55.$_POST["ancode"]."'") === FALSE) 
                                        {
                                            $reply["success"]=FALSE;
                                            $reply["message"]=$conn->error;
                                        }
